...
/* Chỉ lấy đơn đã giao */
const isInvoiceOrder = (status) => String(status).toUpperCase() === "DELIVERED";
...
const buildOptionsMap = (raw) => {
  ...
  if (Array.isArray(raw)) {
    for (const o of raw) {
      ...
      else if (["màu sắc", "màu", "color", "colour"].includes(kk)) out["Màu sắc"] = vv;
      else out[k] = vv;
    }
    return out;
  }
...
const mapOrderToInvoice = (o) => {
  ...
  const totalAmount =
    (o.totalAmount ?? null) != null
      ...
      : Number(o.subtotal ?? 0) - Number(o.discountAmount ?? 0) + Number(o.shippingFee ?? 0);
  ...
  return {
    ...
    items: items.map((it) => {
      ...
      const rawOptions = it?.options ?? it?.orderItemOptions ?? it?.itemOptions;
      const optsMap = buildOptionsMap(rawOptions);
      const variantText =
        optionsToText(optsMap) ||
        (it.size ? `Kích cỡ: ${it.size}` : ""); // fallback nếu BE cũ còn trả size
      return {
        name: it.productName,
...
export default function InvoiceList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const resSeller = await authFetch(apiUrl(API_CONFIG.endpoints.searchSellerByUserId(userId)), {
          method: "GET",
          headers: { Accept: "application/json" },
        });
  ...
  // Fetch delivered orders -> invoices
  useEffect(() => {
    if (!authFetch || !sellerId) return;
    let stop = false;
    (async () => {
      try {
        const res = await authFetch(apiUrl(API_CONFIG.endpoints.ordersBySeller(sellerId)), {
          method: "GET",
          headers: { Accept: "application/json" },
        });
        ...
        const deliveredOnly = list.filter((o) => isInvoiceOrder(o.status)).map(mapOrderToInvoice);

        if (!stop) {
          setInvoices(deliveredOnly);
          setFilteredInvoices(deliveredOnly);
        }
      } catch (e) {
        if (!stop) {
          setInvoices([]);
  ...
  const getSelectedRange = () => {
    ...
    if (selectedDateRange === "7days") return { from: addDays(today, -6), to: today };
    if (selectedDateRange === "30days") return { from: addDays(today, -29), to: today };
  ...
  const dateRangeLabel = () => {
    ...
    const map = { today: "Hôm nay", "7days": "7 ngày qua", "30days": "30 ngày qua", custom: "Tùy chỉnh" };
    const rng = getSelectedRange();
    if (!rng) return map[selectedDateRange];
    return `${map[selectedDateRange]} (${formatDateObj(rng.from)} - ${formatDateObj(rng.to)})`;
  };
      ...
      <body>
        ${list
          .map((inv) => {
            ...
            return `
            <div class="inv">
              <h3>Hóa đơn #${inv.id} <span class="badge">DELIVERED</span></h3>
              <div class="meta">
  ...
  const exportExcel = () => {
    const rows = [
      ...
      ...filteredInvoices.map((inv) => {
        ...
        const dateTxt = inv.orderDate ? `\t${new Date(inv.orderDate).toLocaleDateString("vi-VN")}` : "";
    ...
    a.href = url;
    a.download = `invoices_${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}_${pad(
      now.getHours()
    ...
    <div className="invoice-list-container">
      <div className="invoice-header">
        ...
        <div className="header-actions">
          ...
          <button className="btn-export-invoice" onClick={() => exportInvoices()}>Xuất hóa đơn</button>
        </div>
      </div>
                ...
                <div className="date-quick-grid">
                  {dateRangeOptions
                    ...
                    .map((option) => (
                      <button
                        key={option.key}
                        className={`date-pill ${selectedDateRange === option.key ? "active" : ""}`}
          ...
          <div className="date-tag">
            ...
            {dateRangeLabel() && (
              <span className="date-range-tag">
                <span className="tag-content">Ngày đặt hàng: {dateRangeLabel()}</span>
                <button
        ...
        <div className="table-body">
          {filteredInvoices.map((inv) => (
            <div key={inv.id} className="table-row">
              <div className="col-invoice-id"><strong>{inv.id}</strong></div>
              <div className="col-customer"><div className="customer-name-full">{inv.customerName}</div></div>
              <div className="col-date">{formatDate(inv.orderDate)}</div>
              <div className="col-amount"><strong>{formatCurrency(inv.totalAmount)}</strong></div>
              <div className="col-status"><span className="status-badge status-completed">Hoàn thành</span></div>
              <div className="col-actions">
                ...
                >
                  <svg viewBox="0 0 24 24" width="18" height="18">
                    <path fill="currentColor" d="M6 7V3h12v4H6zm12 2H6c-1.66 0-3 1.34-3 3v5h4v4h10v-4h4v-5c0-1.66-1.34-3-3-3zm-3 10H9v-5h6v5z"/>
                  </svg>
                </button>
              </div>
            </div>
      ...
      {filteredInvoices.length === 0 && (
        <div className="empty-state"><p>Không có hóa đơn nào</p></div>
      )}
    </div>
  );
}
